...
describe 'サイドメニューのテスト' do
  ...
  context '表示内容の確認' do
    ...
    it '募集リンクが表示される: 左上から4番目のリンクが「募集」である' do
      side_menu_link = find_all('a')[3].native.inner_text
  ...
  context 'リンク内容の確認' do
    ...
    it '募集を押すと、募集画面に遷移する' do
      recruit_link = find_all('.side_menu__link')[2].native.inner_text
      recruit_link = recruit_link.gsub(/\n/, '').gsub(/\A\s*/, '').gsub(/\s*\Z/, '')
      click_link recruit_link, match: :first
      is_expected.to eq '/recruits'
    end
    it '卓報告を押すと、卓報告画面に遷移する' do
      evaluation_link = find_all('.side_menu__link')[3].native.inner_text
    ...
    it 'メッセージを押すと、メッセージ画面に遷移する' do
      message_link = find_all('.side_menu__link')[4].native.inner_text
      message_link = message_link.gsub(/\n/, '').gsub(/\A\s*/, '').gsub(/\s*\Z/, '')
      click_link message_link
      is_expected.to eq '/messages'
    end
    it '通知を押すと、通知画面に遷移する' do
      notice_link = find_all('.side_menu__link')[5].native.inner_text
    ...
    it 'スケジュールを押すと、スケジュール画面に遷移する' do
      schedule_link = find_all('.side_menu__link')[6].native.inner_text
    ...
    it '設定を押すと、設定画面に遷移する' do
      setting_link = find_all('.side_menu__link')[7].native.inner_text
